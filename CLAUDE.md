...
There are three distinct content mechanisms — don't confuse them:
...
2. **Blog collection** — `src/content/blog/*.md`, declared in `src/content.config.ts` with a Zod schema (`title`, `description`, `pubDate`). Accessed via `getCollection`/`render`, never via the page glob above. `src/lib/blog.ts` `getPosts()` returns all posts newest-first and is the single source for the blog listing (`src/pages/blog/index.astro`), individual posts (`src/pages/blog/[...slug].astro`, slug = filename), and the RSS feed (`src/pages/rss.xml.ts`).
3. **Papers collection** — `src/content/papers/*.md`, declared in `src/content.config.ts` with a Zod schema (`title`, `description`). Same pattern as the blog collection but simpler (no date, no listing/RSS): `src/lib/papers.ts` `getPapers()` feeds `src/pages/papers/[...slug].astro`, which renders each paper at `/papers/<slug>` (slug = filename).

`src/layouts/Layout.astro` is the shared shell for everything: single-column centered layout (`max-w-prose`), top nav (Home/CV/Blog), `<main class="prose">` slot, footer with social icons, plus all `<head>` meta/OG/canonical/RSS-alternate tags and Person JSON-LD (homepage only). Page content renders inside `prose`, so markdown is styled automatically.
...
- **New blog post**: create `src/content/blog/<slug>.md` with `title`, `description`, and `pubDate`. It appears at `/blog/<slug>` and in the listing/RSS automatically. The visible title is rendered from frontmatter by the page — **do not** add a body H1.
- **New paper**: create `src/content/papers/<slug>.md` with `title` and `description` frontmatter. It appears at `/papers/<slug>` automatically (no listing page, no RSS). The visible title is rendered from frontmatter by the page — **do not** add a body H1.
